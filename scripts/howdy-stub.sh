#!/usr/bin/env bash
# Stands in for howdy-next's native compare helper in mock mode, with the mock
# contract's exit codes — a stub
...
#   MUGLOCK_MOCK=fail  -> 11 scan window expired without a match
#   MUGLOCK_MOCK=nomodel-> 10 no face model enrolled for this user
