...
grep -q "NOPASSWD: /usr/bin/timeout --signal=KILL 12 /usr/bin/howdy compare" <<<"$out" || { echo "FAIL: sudoers line"; exit 1; }
...
grep -q "lock_cmd = qs -c muglock ipc call muglock lock" <<<"$out" || { echo "FAIL: hypridle lock_cmd"; exit 1; }
grep -q "after_sleep_cmd = qs -c muglock ipc call muglock wake" <<<"$out" || { echo "FAIL: hypridle after_sleep_cmd"; exit 1; }
echo PASS
